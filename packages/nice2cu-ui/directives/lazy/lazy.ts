...
import { hasIntersectionObserver, scrollParent } from './dom';
import { LazyOptions, Target, LazyState } from './type';
			...
			(entries) => {
				entries.forEach((entry) => {
					if (entry.isIntersecting) {
						...
						if (manager) {
							if (manager.lazyState === LazyState.loaded) {
								this.removeManager(manager);
								return;
							}
							manager.load();
