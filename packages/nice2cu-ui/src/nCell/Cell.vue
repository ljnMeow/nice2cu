<template>
	<div v-ripple="{ isRipple: isRipple }" :class="[bem.b(), border ? bem.m('border') : '']" @click="cellClick">
		<div v-if="icon || $slots.icon" :class="[bem.e('icon'), ...iconClass]">
			<slot name="icon">
				<nIcon :icon="icon" :size="iconSize" :class-prefix="iconPrefix"></nIcon>
			</slot>
		</div>
		...
		<div v-if="extra || url || link || showArrow || $slots.extra" :class="[bem.e('extra'), ...extraClass]">
			<slot name="extra">
				<nIcon :icon="url || link ? 'n-right' : showArrow ? `n-${showArrow}` : extra" :class-prefix="iconPrefix" :size="iconSize"></nIcon>
			</slot>
		</div>
	</div>
</template>
